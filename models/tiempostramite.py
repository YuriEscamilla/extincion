...
class tiempostramitegestion(models.Model):
    _name = 'tiempos.tramite.extincion'
    _inherit = ['mail.thread', 'mail.activity.mixin']

    RefIdGestion = fields.Many2one(comodel_name='tramite.gestion.extincion', string='Tiempos por etapa')
    ...
    def _dias_proceso(self):
        ...
        for record in self:
            ...
            etapa_obj = self.env['tiempos.tramite.extincion'].search(filtro)
            ...
            '''if etapa_obj.Etapa == 2:
                ...
                if etapa_obj.Origen ==1:
                    ...
                    etapa_inicial_calculo = self.env['tiempos.tramite.extincion'].sudo().search(filtro)
    ...
    def _dias_tramite(self):
        ...
        fecha_etapa = self.env['tiempos.tramite.extincion'].sudo().search(filtro)
        ...
        for record in fecha_etapa:
            ...
            if record.id == ultimo_reg:
                ...
                #fecha de ultima actualizacion del folio
                folio_gestion = [('id','=',idgestion)]
                fecha_autorizacion = self.env['tramite.gestion.extincion'].sudo().search(folio_gestion)
                if fecha_autorizacion.EstatusAsunto == 'autorizado' or fecha_autorizacion.EstatusAsunto == 'rechazado':
                    b = datetime.date(fecha_autorizacion.write_date)
                else:
                    b = datetime.now()
